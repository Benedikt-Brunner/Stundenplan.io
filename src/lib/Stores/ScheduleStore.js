...
function getInsertionPoints(lesson) {
	let lesson_start =
		parseInt(lesson.hours[0].split('-')[0].split(':')[0]) * 100 +
		parseInt(lesson.hours[0].split('-')[0].split(':')[1]);
	let lesson_end =
	...
	hours.forEach((element, index) => {
		if (
			(element[0] <= lesson_start &&
				lesson_start <= element[1] &&
				element[0] <= lesson_end &&
				element[1] <= lesson_end) ||
			(lesson_start <= element[0] &&
				lesson_start <= element[1] &&
				element[0] <= lesson_end &&
				element[1] <= lesson_end) ||
			(lesson_start <= element[0] &&
...
export function insert_merged_lesson(lesson) {
	let merged_lesson = get_friends_lessons().lessons_merged.filter((element) =>
