	...
	for (let friend of friends_list.friends) {
		if (!get(filterList).includes(friend.username)) {
			friend.schedule.forEach((hour_block) => {
				...
				Object.values(hour_block)
					.filter((value) => value !== cur_hour)
					.forEach((lesson, index) => {
						if (lesson.Room === '' && lesson.Subject === '' && lesson.Teacher === '') {
							return;
						}
						friends_lessons.push(
							new Lesson(
								[lesson.Room],
								lesson.Subject,
								lesson.Teacher,
...
export function get_groups() {
	...
	groups =
		groups.length != 0
			? groups.map((group) => {
					let friends_for_group = friends_list.friends.filter(
						(friend) => friend.personal_grouping === group
					);
					return {
						name: group,
