...
export function get_table_pattern() {
	...
	if (Array.isArray(get(comparing).friend)) {
		...
		let friends_days = get(comparing)
			.friend.map((element) => {
				Object.keys(element.schedule[0]).length - 1;
			})
			.reduce((a, b) => {
				return Math.min(a, b);
			});
			...
			_.forEach((_, index2) => {
				...
				overlaps_arr.forEach((element, index3) => {
					if (element === overlaps.no_overlap) {
						...
						is_lesson(own_day) &&
						is_lesson_from_arr(
							get(comparing).friend.map((f) => {
								return f.schedule[index3][`Day${index2 + 1}`];
							})
						) &&
						is_same_lesson_from_arr(
							own_day,
							get(comparing).friend.map((f) => {
								return f.schedule[index3][`Day${index2 + 1}`];
							})
						...
						element === overlaps.partial_overlap &&
						(is_lesson(own_day) ||
							is_lesson_from_arr(
								get(comparing).friend.map((f) => {
									return f.schedule[index3][`Day${index2 + 1}`];
		...
		_.forEach((_, index2) => {
			...
			overlaps_arr.forEach((element, index3) => {
				if (element === overlaps.no_overlap) {
					...
					is_same_lesson(own_day, get(comparing).friend.schedule[index3][`Day${index2 + 1}`])
				) {
					res = overlap_colors.overlap_same_class;
				} else if (
					element === overlaps.full_overlap &&
...
function determine_overlap(own_hour, friend_hour) {
	...
	if (friend_hour[0] <= own_hour[0] && friend_hour[1] >= own_hour[1]) {
		...
		(friend_hour[0] >= own_hour[0] && friend_hour[1] <= own_hour[1]) ||
		(friend_hour[0] <= own_hour[0] &&
			friend_hour[1] <= own_hour[1] &&
			friend_hour[1] >= own_hour[0]) ||
		(friend_hour[0] >= own_hour[0] &&
			friend_hour[0] <= own_hour[1] &&
			friend_hour[1] >= own_hour[1])
	) {
...
function is_same_lesson(own_lesson, friend_lesson) {
	return (
		own_lesson.Subject === friend_lesson.Subject &&
		own_lesson.Teacher === friend_lesson.Teacher &&
		own_lesson.Room === friend_lesson.Room
	);
}
